...
function handleError(agent, task, err) {
  ...
  if (c.blocking) {
    ...
    const title = c.kind === "quota" ? `Meridian rate limit hit (${GEMINI_MODEL})`
      : c.kind === "config" ? `Invalid model: "${GEMINI_MODEL}"`
      : `Meridian ${c.kind} error (${GEMINI_MODEL})`;
    const hint = c.kind === "quota"
      ? "Both the heavy and light tiers were rate-limited. Wait for your Claude Code subscription's limit to refresh, then press ALL HANDS. If it persists, lower TICK_MS / AUTO usage, or check Meridian."
      : c.kind === "config"
      ? "The model name is unknown. Set ANTHROPIC_MODEL (heavy) / ANTHROPIC_FAST_MODEL (light) to valid Claude model ids, e.g. 'claude-sonnet-4-6' and 'claude-haiku-4-5-20251001'."

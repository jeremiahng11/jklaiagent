                ...
                <div key={m.id} style={{ ...SS.memberCard, borderColor: `${m.color}55` }}>
                  ...
                  <div style={SS.memberBadges}>
                    {m.human
                      ...
                      : <><span style={SS.metaBadge}>◇ {model || "claude"}</span><span style={SS.metaBadge}>⌁ {m.cadence}</span></>}
                  </div>
                ...
                <div style={SS.autoBanner}>
                  <span style={{ flex: 1 }}>
                    <Bot size={13} style={{ verticalAlign: -2, marginRight: 5 }} /><b>AUTO is ON — visual demo.</b> {demoModel
                      ...
                      : <>Demo tasks are simulated — <b>no model calls, costs nothing</b>. Only tasks <b>you</b> assign use the API.</>}
                  </span>
        ...
        <div style={SS.modalBg} onClick={() => setSelected(null)}>
          <div style={SS.modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={SS.modalActions}>
              {["failed", "blocked", "in_progress", "review", "planning"].includes(selectedTask.status) && <button style={SS.continueBtn} onClick={() => retryTask(selectedTask.id).then(() => alert("Jay Jay is re-dispatching this task — watch the Visual office.")).catch((e) => alert("Couldn't continue: " + e.message + (/not found/i.test(e.message) ? " (the task no longer exists — re-assign it fresh)" : "")))}><RotateCw size={13} /> CONTINUE TASK</button>}
              {(() => {
